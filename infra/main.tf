...
resource "null_resource" "install_dependencies_delete" {
  provisioner "local-exec" {
    command = "pip install -r ../functions/delete-note/requirements.txt -t ../functions/delete-note/"
  }
    triggers = {
      dependencies_versions = filemd5("../functions/delete-note/requirements.txt")
      source_versions = filemd5("../functions/delete-note/main.py")
  }
}

resource "null_resource" "install_dependencies_get" {
  provisioner "local-exec" {
    command = "pip install -r ../functions/get-notes/requirements.txt -t ../functions/get-notes/"
  }
    triggers = {
      dependencies_versions = filemd5("../functions/get-notes/requirements.txt")
      source_versions = filemd5("../functions/get-notes/main.py")
  }
}
...
resource "aws_iam_role_policy_attachment" "dynamodb_policy_attachment_delete"{
  policy_arn = aws_iam_policy.dynamodb_policy.arn
  role = aws_iam_role.lambda_delete.name
}

resource "aws_iam_role_policy_attachment" "dynamodb_policy_attachment_get"{
  policy_arn = aws_iam_policy.dynamodb_policy.arn
  role = aws_iam_role.lambda_get.name
}
